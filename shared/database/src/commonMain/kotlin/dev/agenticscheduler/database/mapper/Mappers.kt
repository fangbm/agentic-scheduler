...
internal fun PlanningProfile.toRecord() = when (val value=configuration) { PlanningProfileConfiguration.Unconfigured -> PlanningProfileRecord(id.value,name); is PlanningProfileConfiguration.Configured -> PlanningProfileRecord(id.value,name,"CONFIGURED",value.timeZone.id,value.minimumFocusBlock.toIsoString(),value.preferredFocusBlock.toIsoString(),value.maximumFocusBlock.toIsoString(),value.allDayEventPolicy.name) }
internal fun PlanningProfile.windowRecords() = (configuration as? PlanningProfileConfiguration.Configured)?.weeklyAvailability?.map { PlanningProfileAvailabilityWindowRecord(id.value,it.dayOfWeek.name,it.start.toString(),it.endExclusive.toString()) }.orEmpty()
internal fun PlanningProfileRecord.toDomain(windows:List<PlanningProfileAvailabilityWindowRecord> = emptyList()) = PlanningProfile(PlanningProfileId(id), name, when(configurationState) { "UNCONFIGURED" -> PlanningProfileConfiguration.Unconfigured; "CONFIGURED" -> PlanningProfileConfiguration.Configured(TimeZone.of(requireNotNull(timeZoneId)),windows.map { WeeklyAvailabilityWindow(DayOfWeek.valueOf(it.dayOfWeek),LocalTime.parse(it.startLocalTime),LocalTime.parse(it.endLocalTimeExclusive)) }.toImmutableList(),Duration.parseIsoString(requireNotNull(minimumFocusDurationIso)),Duration.parseIsoString(requireNotNull(preferredFocusDurationIso)),Duration.parseIsoString(requireNotNull(maximumFocusDurationIso)),AllDayEventPolicy.valueOf(requireNotNull(allDayEventPolicy))); else -> error("Unknown PlanningProfile configuration state: $configurationState") })
