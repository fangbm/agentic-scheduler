...
A local-first Kotlin Multiplatform scheduler. D8 client E2EE, opaque sync-server relay, and production secure-storage adapters are implemented and completion-accepted; OD-012 local SQLite encryption remains a separate production-sensitive-data release gate.

The repository includes the completed D1–D4 foundations, completed D5-01 Calendar/Agenda surface, implemented D5-02 Event/Task creation-editing flow, completed D6 deterministic Planner/PlanBranch work, completed D7 mutation/history/causality foundation, and completed D8 encrypted multi-device sync. D5-02/D6.5 build and packaging verification passed locally on 2026-09-20; Desktop dogfood passed interactively, and the Android Planner surface plus New Task dialog respond on the API 36 emulator. Full Android Planner input smoke remains pending. D8 completion includes app-level catch-up wiring, direct/phone-relay route equivalence, Recovery/Revocation PostgreSQL E2E, plaintext-leak scanning, and Android/Wear Keystore, Windows DPAPI, Linux Secret Service, and server CI acceptance.
...
- `shared:database`: Room 3 / SQLite KMP persistence implementation, including exported client schema v11, records, mappers, repositories, and platform database builders.
